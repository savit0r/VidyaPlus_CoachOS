...
interface BatchesGridLayerProps {
  onNavigate: (depth: DrillDepth, data?: { batchId?: string }) => void;
}

export default function BatchesGridLayer({ onNavigate }: BatchesGridLayerProps) {
    ...
    <div className="space-y-6" onClick={() => setActiveMenu(null)}>
      ...
      {batches.length === 0 ? (
        <div className="mint-card py-12 text-center border-dashed">
          ...
          <button onClick={() => { setEditBatch(null); setShowModal(true); }} className="text-brand-green font-bold hover:underline">
            + Create First Batch
          </button>
        </div>
      ) : (
